...
def SM2encrypt(key,publickey,privatekey):
    with open(key,'rb') as v:
        SM4key = v.read()
    v.close()
...
def SM2decrypt(key,publickey,privatekey):
    with open(key,'rb') as v:
...
def init(SM4key,pubkey,prikey,now_time,path,videopath):
    sign = SM3sign(SM4key)
...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        init(self.SM4key,self.pubkey,self.prikey,self.now_time,self.path,self.videopath)
